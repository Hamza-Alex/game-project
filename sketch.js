...
var verwerkBotsing = function() {
  ...
  if (powerUpActief && 
    ...
    abs(spelerY - powerUpY) < 50){
      ...
      if (keuze === 0) {
          huidigeBuff = "schild"
          buffStartTijd = millis(); // speler heeft schild tegen vuurballen
      } 

      if (keuze === 1) {
        huidigeBuff = "snelheid"
        buffStartTijd = millis();
        spelerSnelheid = spelerSnelheid + 5; // speler rent met 5 sneller 
      }
...
if (powerUpActief) {
  image(powerUpImg, powerUpX - 25, powerUpY -25, 50, 50)
}
      
//Visueel schild effect
if (huidigeBuff === "schild" && millis () - buffStartTijd < 1000){
  noFill();
  stroke("cyan");
  strokeWeight(4);
  ellipse(spelerX, spelerY, 80, 80) // dit is de schild om de sppeler heen
  noStroke();
}
...
// Vijand 3
  ...
  // Vijand 4
  if (Vijand4 && abs(spelerX - Vijand4X) < 50 && abs(spelerY - Vijand4Y) < 50) {
    botsing = true;
  }

  if (botsing) {
    if (huidigeBuff === "schild") {
     console.log("Botsing, maar beschermd door schild! Levens blijven:" + levens ); // niks doen, speler heeft schild, dus geen leven weg
    } else {
    levens--;
    HartjeMinder.play();
